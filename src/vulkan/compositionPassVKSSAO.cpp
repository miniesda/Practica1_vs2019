...
CompositionPassVKSSAO::CompositionPassVKSSAO(                             
    ...
    m_in_noise_attachment(i_in_noise_attachment),
...
bool CompositionPassVKSSAO::initialize()
{
...
void CompositionPassVKSSAO::shutdown()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
VkCommandBuffer CompositionPassVKSSAO::draw( const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    UtilsVK::beginRegion( current_cmd, "Ambient Oclussion Pass", Vector4f( 0.5f, 0.0f, 0.0f, 1.0f ) );
...
void CompositionPassVKSSAO::createFbo()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void CompositionPassVKSSAO::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void CompositionPassVKSSAO::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void CompositionPassVKSSAO::createDescriptorLayout()
{
...
void CompositionPassVKSSAO::createDescriptors()
{
    ...
    {   
        ...
        std::array<VkDescriptorBufferInfo,2> binfo;
        binfo[0].buffer    = m_runtime.getPerFrameBuffer()[ i ];
        binfo[0].offset    = 0;
        binfo[0].range     = sizeof( PerFrameData );

        //Cambiar despues
        binfo[1].buffer = m_runtime.getSSAOBuffer();
        binfo[1].offset = 0;
        binfo[1].range = sizeof( SSAOData );
        ...
        image_infos[ 2 ].sampler     = m_in_noise_attachment.m_sampler;
        image_infos[ 2 ].imageView   = m_in_noise_attachment.m_image_view;
        ...
        set_write[ 4 ].pBufferInfo = &binfo[1];
