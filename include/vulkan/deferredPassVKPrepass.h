...
namespace MiniEngine
{
    ...
    class DeferredPassVKPrepass final : public RenderPassVK
    {
    public:
        DeferredPassVKPrepass(
            const Runtime& i_runtime, 
            const ImageBlock& i_depth_buffer);
        virtual ~DeferredPassVKPrepass();
    ...
    private:
        DeferredPassVKPrepass( const DeferredPassVKPrepass& ) = delete;
        DeferredPassVKPrepass& operator=(const DeferredPassVKPrepass& ) = delete;
        ...
        std::array<MaterialPipeline, 1> m_pipelines; //one by material      //one diffuse, one microfacets
