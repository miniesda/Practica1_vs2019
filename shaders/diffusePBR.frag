...
void main() {
    ...
    out_normal          = vec4( normalize( f_normal ) * 0.5f + 0.5f, 0.0f );    //acordarse de 0.5
    out_position_depth  = vec4( f_position, linearDepth(gl_FragCoord.z) );
    out_material        = vec4( per_object_data.objects[ f_instance ].m_metallic_roughness.xy, 1.0, 1.0 ); //0 for diffuse
}